#ifndef HYDRO_RSOLVERS_HLLC_HYD_SPLIT_HPP_
#define HYDRO_RSOLVERS_HLLC_HYD_SPLIT_HPP_
//========================================================================================
// AthenaXXX astrophysical plasma code
// Copyright(C) 2020 James M. Stone <jmstone@ias.edu> and the Athena code team
// Licensed under the 3-clause BSD License (the "LICENSE")
//========================================================================================
//! \file hllc_hyd_split.hpp
//! \brief Single-face HLLC Riemann solver for the split-kernel flux path.
//! Templated on direction (ivx = IVX|IVY|IVZ).
//! Math is identical to the team-policy HLLC wrapper in hllc_hyd.hpp.
...
void SingleFaceHLLC(const EOS_Data &eos,
  ...
  Real wl_idn = wl(m, IDN, kb, jb, ib);
  Real wl_ivx = wl(m, ivx, kb, jb, ib);
  Real wl_ivy = wl(m, ivy, kb, jb, ib);
  Real wl_ivz = wl(m, ivz, kb, jb, ib);

  Real wr_idn = wr(m, IDN, kb, jb, ib);
  Real wr_ivx = wr(m, ivx, kb, jb, ib);
  Real wr_ivy = wr(m, ivy, kb, jb, ib);
  Real wr_ivz = wr(m, ivz, kb, jb, ib);

  Real wl_ipr = eos.IdealGasPressure(wl(m, IEN, kb, jb, ib));
  Real wr_ipr = eos.IdealGasPressure(wr(m, IEN, kb, jb, ib));
...
#endif  // HYDRO_RSOLVERS_HLLC_HYD_SPLIT_HPP_
