...
//----------------------------------------------------------------------------------------
//! \fn ReconDispatch()
//! \brief Dispatch the correct version of a function templated over a reconstruction
//! operator.
template<class F>
void ReconDispatch(ReconstructionMethod recon, F f) {
  if (recon == ReconstructionMethod::dc) {
    f(ReconDCTag());
  } else if (recon == ReconstructionMethod::plm) {
    f(ReconPLMTag());
  } else if (recon == ReconstructionMethod::ppm4) {
    f(ReconPPM4Tag());
  } else if (recon == ReconstructionMethod::ppmx) {
    f(ReconPPMXTag());
...
KOKKOS_INLINE_FUNCTION
void ReconCellTemplate(const Recon &recon, const EOS_Data &eos,
               const bool apply_floors,
               const int m, const int k, const int j, const int i,
               const int nvars,
               const DvceArray5D<Real> &q,
  ...
  for (int n = 0; n < nvars; ++n) {
    Real ql_val, qr_val;
    if constexpr (std::is_same_v<Recon, ReconDCTag>) {
      ql_val = q(m, n, k, j, i);
      qr_val = q(m, n, k, j, i);
    } else if (std::is_same_v<Recon, ReconPLMTag>) {
      PLM(q(m, n, k - dk, j - dj, i - di),
          q(m, n, k,      j,      i),
          q(m, n, k + dk, j + dj, i + di),
          ql_val, qr_val);
    } else if (std::is_same_v<Recon, ReconPPM4Tag>) {
      PPM4(q(m, n, k - 2*dk, j - 2*dj, i - 2*di),
           q(m, n, k -   dk, j -   dj, i -   di),
           q(m, n, k,        j,        i),
           q(m, n, k +   dk, j +   dj, i +   di),
           q(m, n, k + 2*dk, j + 2*dj, i + 2*di),
           ql_val, qr_val);
    } else if (std::is_same_v<Recon, ReconPPMXTag>) {
      PPMX(q(m, n, k - 2*dk, j - 2*dj, i - 2*di),
           q(m, n, k -   dk, j -   dj, i -   di),
           q(m, n, k,        j,        i),
           q(m, n, k +   dk, j +   dj, i +   di),
    ...
    } else if (std::is_same_v<Recon, ReconWENOZTag>) {
      WENOZ(q(m, n, k - 2*dk, j - 2*dj, i - 2*di),
            q(m, n, k -   dk, j -   dj, i -   di),
            q(m, n, k,        j,        i),
            q(m, n, k +   dk, j +   dj, i +   di),
